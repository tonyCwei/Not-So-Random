...
function Header({ showForm, setShowForm }) {
  const appTitle = "Not so Random";
  return (
    <>
      <header className="header">
        <div className="logo">
          <img src="logo.png" height="68" width="68" alt="Not so Random Logo" />
          <h1>{appTitle}</h1>
        </div>
...
function NewFactForm({ setFacts, setShowForm }) {
  ...
  async function handleSubmit(e) {
    ...
    if (text && isValidHttpUrl(source) && category) {
      ...
      // 3.1 Upload fact to supabase and receive the new fact obj
      setIsUploading(true);
      ...
      //4. Add new fact to the UI: add the fact ot state
      if (!error) {
  ...
  return (
    <form className="fact_form" onSubmit={handleSubmit}>
      ...
      />
      <span className="wordlimit">{200 - textLength}</span>
      <input
        type="text"
        placeholder="http://"
        value={source}
...
function FactList({ facts, setFacts }) {
  ...
  return (
    <section className="facts-list-box">
